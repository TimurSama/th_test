...
from config import DATABASE_PATH, SUBSCRIPTION_LEVELS, EXCHANGES, ENABLE_MINUTE_NOTIFICATIONS
...
from services.price_news import PriceNewsService
...
class PriceMonitor:
    def __init__(self):
        ...
        self.news_service = PriceNewsService()
    ...
    async def should_send_notification(self, user_id: int, level: str) -> bool:
        # If minute notifications are enabled, send every minute
        if ENABLE_MINUTE_NOTIFICATIONS:
            async with aiosqlite.connect(DATABASE_PATH) as db:
                try:
                    cursor = await db.execute("""
                    ...
                    next_notification = last_notification + timedelta(minutes=1)
                    return datetime.utcnow() >= next_notification
                except Exception as e:
                    logger.error(f"Error checking notification time: {e}")
                    return True
        else:
            # Use subscription-based intervals (original behavior)
            async with aiosqlite.connect(DATABASE_PATH) as db:
                try:
                    cursor = await db.execute("""
                        SELECT last_notification_time FROM users WHERE user_id = ?
    ...
    async def start_monitoring(self, bot_instance=None):
        ...
        while self.running:
            try:
                ...
                if comparison:
                    # Check for significant price changes and create news
                    news_items = await self.news_service.detect_significant_changes(comparison)
                    if news_items:
                        await self.news_service.publish_price_news(news_items)
                        logger.info(f"Published {len(news_items)} price news items")
        ...
        for level in ['free', 'pro', 'premium']:
            ...
            for user_data in users:
                ...
                if await self.should_send_notification(user_id, level):
                    try:
                        # Format message with HTML for better structure
                        message = format_price_table(comparison, self.top_symbols, use_html=True)
                        ...
                        try:
                            message = format_price_table(comparison, self.top_symbols, use_html=False)
